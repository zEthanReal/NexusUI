...
function NexusUI:CreateWindow(config)
    ...
    -- Runs before any frame is created so W/H are always safe
    local function getViewport()
        -- Camera viewport is the most reliable cross-platform source
        local cam = workspace:FindFirstChildOfClass("Camera")
        if cam then return cam.ViewportSize end
        return Vector2.new(1280, 720)
    end
    local vp  = getViewport()
    -- Reserve 24px margin on each side so window never bleeds off-screen
    local margin = 24
    local W = math.min(cfg.Size.X, vp.X - margin * 2)
    local H = math.min(cfg.Size.Y, vp.Y - margin * 2)
    ...
    ScreenGui.IgnoreGuiInset   = false   -- false = respects top bar; prevents top-edge bleed
    ...
    local ShadowImg = Make("ImageLabel", {
        ...
        ImageTransparency  = 1,        -- starts invisible; fades in via entry animation
    ...
    -- Entry animation — window scales up from 88% and fades in
    ...
    -- ── NOTIFICATION LAYER ───────────────────────────────────
    -- Anchored to bottom-right of the screen regardless of resolution
    ...
    do
        ...
        -- Clamp helper: keeps the window fully within viewport
        local function clampPosition(rawX, rawY)
            local vps  = getViewport()
            local hw   = MainFrame.AbsoluteSize.X / 2
            local hh   = MainFrame.AbsoluteSize.Y / 2
            local minX = hw
            local maxX = vps.X - hw
            local minY = hh
            local maxY = vps.Y - hh
            return math.clamp(rawX, minX, maxX),
                   math.clamp(rawY, minY, maxY)
        end

        local function syncShadow(pos)
            ShadowImg.Position = UDim2.new(pos.X.Scale, pos.X.Offset,
        ...
        local function moveDrag(input)
            ...
            local cx, cy = clampPosition(rawX, rawY)
            local newPos = UDim2.new(frameStart.X.Scale, cx,
                                     frameStart.Y.Scale, cy)
    ...
    local shadowFullSize = UDim2.new(0, W + 50, 0, H  + 50)
    local shadowMiniSize = UDim2.new(0, W + 50, 0, 52 + 50)

    MinBtn.MouseButton1Click:Connect(function()
        Window.Minimized = not Window.Minimized
        if Window.Minimized then
            -- Hide content FIRST so it doesn't clip-flash during shrink
            ContentFrame.Visible = false
            Tween(MainFrame, { Size = miniSize }, 0.32, Enum.EasingStyle.Quart)
            -- Shadow shrinks and fades out completely
            Tween(ShadowImg, { Size = shadowMiniSize, ImageTransparency = 1 }, 0.28)
            MinBtn.Text = "⬜"
        else
            ContentFrame.Visible = true
            Tween(MainFrame, { Size = fullSize }, 0.38, Enum.EasingStyle.Back)
            -- Shadow grows back and fades in
            Tween(ShadowImg, { Size = shadowFullSize, ImageTransparency = 0.45 }, 0.38, Enum.EasingStyle.Back)
            MinBtn.Text = "─"
        end
    end)
    ...
    UserInputService.InputBegan:Connect(function(input, proc)
        ...
        if input.KeyCode == cfg.ToggleKey then
            ...
            -- Shadow must also hide/show with the window
