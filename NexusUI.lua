...
║         PC · Mobile · Tablet  ·  Zero Cut-off  ·  Clean API                 ║
║                                                                              ║
║  WHAT'S NEW IN v6:                                                           ║
║    ► 2 new themes  : Sunset · Aqua  (9 total)                               ║
║    ► Card redesign : glass feel, hover accent strip, 54px default           ║
║    ► Title bar     : slim version badge, sharper accent stripe              ║
║    ► Sidebar       : gradient active pill, icon glow on active              ║
║    ► Notifications : 88px, animated icon, X close button                   ║
║    ► New elements  : CreateMultiDropdown · CreateStepper                    ║
║    ► Button        : SetLoading(bool) support                               ║
║    ► Window:Alert() — full modal overlay (confirm/cancel)                  ║
║    ► Window:GetVersion() · Window:ResetConfig() · Window:SetVisible()      ║
╚══════════════════════════════════════════════════════════════════════════════╝
  ...
  QUICK START:
    ...
    local Win = NexusUI:CreateWindow({
        Title      = "My Script",
        SubTitle   = "v1.0",
        Theme      = "Dark",       -- Dark|Midnight|Ocean|Purple|Rose|Neon|Sunset|Aqua|Light
        ToggleKey  = Enum.KeyCode.RightShift,
        ToggleIcon = "",           -- rbxassetid://... or "" for default ☰
    ...
    Tab:CreateSlider({
        Name = "Walk Speed", Min = 16, Max = 250, Default = 16, Suffix = " st",
    ...
    Tab:CreateDropdown({
        Name = "Mode", Options = {"Easy","Normal","Hard"}, Default = "Normal",
        Callback = function(v) print("Mode:", v) end,
    })
    Tab:CreateMultiDropdown({
        Name = "Perks", Options = {"Speed","Jump","Regen","Shield"},
        Callback = function(selected) print(table.concat(selected,", ")) end,
    })
    Tab:CreateStepper({
        Name = "Kills", Min = 0, Max = 9999, Default = 0, Increment = 1,
        Callback = function(v) print("Kills:", v) end,
    })
    Tab:CreateProgressBar({ Name = "Health", Default = 75, Suffix = "%" })
    Tab:CreateButton({ Name = "Teleport Hub", Callback = function() end })
    Win:Notify({ Title = "Loaded", Text = "NexusUI v6 ready!", Type = "Success" })
    Win:Alert({
        Title = "Confirm Action", Text = "Are you sure you want to reset?",
...
function NexusUI:CreateWindow(cfg)
    ...
    task.defer(function()
        Tween(Root,        {Size = UDim2.new(0,W,0,H)},    0.46, Enum.EasingStyle.Back,  Enum.EasingDirection.Out)
        Tween(WinFrame,    {BackgroundTransparency = 0},    0.38)
        Tween(WinStroke,   {Transparency = 0},              0.38)
        Tween(ShadowOuter, {ImageTransparency = 0.52},      0.56)
        Tween(ShadowInner, {ImageTransparency = 0.70},      0.56)
    end)
    ...
    local function showUI()
        ...
        Tween(ShadowOuter,{ImageTransparency=0.52},      0.44)
        Tween(ShadowInner,{ImageTransparency=0.70},      0.44)
    end
    local function hideUI()
        Tween(Root,       {Size=UDim2.new(0,W*0.90,0,H*0.90)},0.24,Enum.EasingStyle.Back,Enum.EasingDirection.In)
        Tween(WinFrame,   {BackgroundTransparency=1},   0.22)
        Tween(WinStroke,  {Transparency=1},             0.22)
        Tween(ShadowOuter,{ImageTransparency=1},        0.18)
        Tween(ShadowInner,{ImageTransparency=1},        0.18)
        task.delay(0.27, function()
            Root.Visible = false
            ToggleWrap.Visible = true
            TBtn.Size = UDim2.new(0,TB_SZ*0.65,0,TB_SZ*0.65)
            Tween(TBtn,{Size=UDim2.new(0,TB_SZ,0,TB_SZ)},0.34,Enum.EasingStyle.Back,Enum.EasingDirection.Out)
        end)
    end
    ...
    function Window:CreateTab(name, iconId)
        ...
        function Tab:CreateToggle(c)
            ...
            local function setVal(v, silent)
                ...
                if v then
                    Tween(track,{BackgroundColor3=T.ToggleOn},0.20,Enum.EasingStyle.Quart)
                    task.delay(0.07,function()
                        Tween(knob,{Position=UDim2.new(0,TW-sqW-INS,0.5,-KD/2),
                            Size=UDim2.new(0,KD,0,KD)},0.20,Enum.EasingStyle.Quart)
                    end)
                else
                    Tween(track,{BackgroundColor3=T.ToggleOff},0.20,Enum.EasingStyle.Quart)
                    task.delay(0.07,function()
        ...
        function Tab:CreateButton(c)
            ...
            return {
                -- SetLoading(true) disables clicks and shows spinning dot
        ...
        function Tab:CreateSlider(c)
            ...
            return {Set=function(v) applyAlpha((v-mn)/(mx-mn)) end,Get=function() return curVal end}
        end
        ...
        function Tab:CreateDropdown(c)
            ...
            local OPT_H=34; local MAX_H=180
            ...
            local function rebuild(o)
                ...
                for i,opt in ipairs(o) do
                    ...
                    local ob=New("TextButton",{
                        ...
                        BackgroundColor3=sel and T.SurfaceAlt or T.Surface,
                        ...
                        AutoButtonColor=false, LayoutOrder=i, ZIndex=22, Parent=list,
                    }); Corner(ob,8)
                    ob.MouseEnter:Connect(function() Tween(ob,{BackgroundTransparency=0},0.10) end)
                    ob.MouseLeave:Connect(function() Tween(ob,{BackgroundTransparency=sel and 0 or 1},0.10) end)
                    local function pick()
                        selVal=opt; selLbl.Text=opt; close(); cb(opt); rebuild(o)
                    end
            ...
            return {
                ...
                Refresh=function(o) opts=o;rebuild(o) end,
            }
        end
        ...
        function Tab:CreateMultiDropdown(c)
            ...
            local OPT_H=36; local MAX_H=190
            local isOpen=false
            local optBtns = {}
            ...
            for i, opt in ipairs(opts) do
                ...
                local hitBtn = New("TextButton", {
                    Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1,
                    Text = "", AutoButtonColor = false, ZIndex = 24, Parent = row,
                })
                local function pick()
                    if selected[opt] then
                        ...
                        -- Enforce max
                        if maxSel>0 and #getSelected()>=maxSel then return end
                        selected[opt] = true
                    end
                    refreshRow(opt)
                    updateCount()
        ...
        function Tab:CreateStepper(c)
            ...
            return {
                Set=function(v) curVal=math.clamp(v,mn,mx); valLbl.Text=tostring(curVal)..suf end,
        ...
        function Tab:CreateProgressBar(c)
            ...
            local function setVal(v)
                ...
                Tween(fill,{Size=UDim2.new(v/100,0,1,0)},0.32,Enum.EasingStyle.Quad)
                vLbl.Text = tostring(math.floor(v))..suf
            end
            return {Set=setVal,Get=function() return curVal end}
        end
        ...
        function Tab:CreatePicker(c)
            ...
            return {Set=function(v) setActive(v,false) end,Get=function() return selVal end}
        end
