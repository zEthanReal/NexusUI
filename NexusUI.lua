...
function NexusUI:CreateWindow(cfg)
    ...
    -- Both buttons live in a BtnGroup that is fully inside TitleBar.
    -- Positions are absolute so WinFrame's ClipsDescendants never touches them.
    local BTN_W   = 28
    local BTN_H   = 28
    local BTN_R   = 10   -- right margin from window right edge
    local BTN_GAP = 8    -- gap between the two buttons
    -- Total group width = BTN_W*2 + BTN_GAP = 64px
    -- Right edge of group = W - BTN_R = flush 10px from right

    local BtnGroup = New("Frame", {
        Size     = UDim2.new(0, BTN_W*2 + BTN_GAP, 0, BTN_H),
        Position = UDim2.new(1, -(BTN_W*2 + BTN_GAP + BTN_R), 0.5, -BTN_H/2),
        BackgroundTransparency = 1,
        ZIndex   = 7,
        Parent   = TitleBar,
    })

    local function CtrlBtn(xPos, bg, sym, hoverColor)
    ...
    local MinBtn   = CtrlBtn(0,             T.SurfaceAlt,                "─", T.Accent)
    local CloseBtn = CtrlBtn(BTN_W+BTN_GAP, Color3.fromRGB(255, 74, 74), "✕", Color3.fromRGB(220,45,45))
    ...
    MinBtn.MouseButton1Click:Connect(function()
        ...
        if Window.Minimized then
            Body.Visible = false   -- hide content before shrink
            Tween(Root,   {Size = UDim2.new(0, W, 0, TITLE_H)},           0.27, Enum.EasingStyle.Quart)
            Tween(Shadow, {Size = UDim2.new(1, 58, 0, TITLE_H + 58),
                           ImageTransparency = 1},                          0.24)
            MinBtn.Text = "⬜"
        else
            Tween(Root,   {Size = UDim2.new(0, W, 0, FULL_H)},            0.36, Enum.EasingStyle.Back)
            Tween(Shadow, {Size = UDim2.new(1, 58, 1, 58),
                           ImageTransparency = 0.40},                       0.36, Enum.EasingStyle.Back)
    ...
    CloseBtn.MouseButton1Click:Connect(function()
        Tween(Root,     {Size = UDim2.new(0, W*0.86, 0, H*0.86)}, 0.28, Enum.EasingStyle.Back, Enum.EasingDirection.In)
        Tween(WinFrame, {BackgroundTransparency = 1},               0.26)
        Tween(Shadow,   {ImageTransparency = 1},                    0.22)
        task.delay(0.33, function() Screen:Destroy() end)
    end)
    ...
    UserInputService.InputBegan:Connect(function(inp, proc)
        ...
        if inp.KeyCode == toggleKey then
            ...
            -- NotifFrame stays accessible even when window is hidden
        end
    end)
