...
local function Ripple(frame, px, py)
    -- Subtle ripple: starts nearly invisible, stays small, exits fast.
    -- 0.88 start-transparency = barely-there flash rather than a white bloom.
    local r = New("Frame", {
    ...
    Corner(r, 999)
    -- 1.3× instead of 2.5× keeps the circle contained inside the card
    local d = math.max(frame.AbsoluteSize.X, frame.AbsoluteSize.Y) * 1.3
...
function NexusUI:CreateWindow(cfg)
    ...
    function Window:CreateTab(name, iconId)
        ...
        function Tab:CreateToggle(c)
            ...
            --[[
                Track dimensions (iOS proportions):
                  Width  = 51 px  (was 46)
                  Height = 30 px  (was 26)
                  Corner = 15     (exactly half height → perfect pill)

                Knob dimensions:
                  Diameter = 24 px  (was 20)
                  OFF pos  = left  3 px inset
                  ON  pos  = right 3 px inset  → 51-24-3 = 24 from left
                  Corner   = 12    (perfect circle)

                Hit area: only the right ~70px of the card so tapping
                the label text side never accidentally fires the toggle.
            ]]
            local TRACK_W  = 51
            local TRACK_H  = 30
            local KNOB_D   = 24
            local INSET    = 3   -- gap between knob edge and track wall

            -- Track
            local track = New("Frame", {
                Size             = UDim2.new(0, TRACK_W, 0, TRACK_H),
                Position         = UDim2.new(1, -(TRACK_W + 12), 0.5, -TRACK_H/2),
            ...
            -- Inner shadow ring so the track looks recessed
            Stroke(track, Color3.fromRGB(0,0,0), 1, 0.82)

            -- Knob
            local knobOFF = UDim2.new(0, INSET,           0.5, -KNOB_D/2)
            local knobON  = UDim2.new(0, TRACK_W-KNOB_D-INSET, 0.5, -KNOB_D/2)
            local knob = New("Frame", {
                Size             = UDim2.new(0, KNOB_D, 0, KNOB_D),
                Position         = def and knobON or knobOFF,
                BackgroundColor3 = Color3.new(1, 1, 1),
                BorderSizePixel  = 0,
                ZIndex           = 9,
                Parent           = track,
            })
            Corner(knob, KNOB_D / 2)
            -- Subtle drop shadow on knob so it pops off the track
            Stroke(knob, Color3.fromRGB(0,0,0), 1, 0.88)

            local value   = def
            local tweening = false   -- debounce rapid taps
            ...
            local function setVal(v, silent)
                ...
                -- Squish knob wide on press (tactile feel), snap back on release
                local squishW = KNOB_D + 5
                Tween(knob, {Size = UDim2.new(0, squishW, 0, KNOB_D)}, 0.08)

                if v then
                    Tween(track, {BackgroundColor3 = T.ToggleOn},  0.20, Enum.EasingStyle.Quart)
                    task.delay(0.08, function()
                        -- Move to ON position; account for squish width so it lands flush
                        Tween(knob, {
                            Position = UDim2.new(0, TRACK_W - squishW - INSET, 0.5, -KNOB_D/2),
                            Size     = UDim2.new(0, KNOB_D, 0, KNOB_D),
                ...
                task.delay(0.30, function() tweening = false end)
                if not silent then cb(v) end
            end

            -- Hit area covers only the track side of the card
            -- so tapping the label text cannot accidentally toggle.
            local trackHit = New("TextButton", {
                Size             = UDim2.new(0, TRACK_W + 20, 1, 0),
                Position         = UDim2.new(1, -(TRACK_W + 30), 0, 0),
                BackgroundTransparency = 1,
                Text             = "",
                AutoButtonColor  = false,
                ZIndex           = 10,
            ...
            trackHit.MouseButton1Click:Connect(function() setVal(not value) end)
            trackHit.InputBegan:Connect(function(i)
                if i.UserInputType == Enum.UserInputType.Touch then setVal(not value) end
            end)
            -- Also allow clicking anywhere on the card (convenience)
            local cardHit = HitBtn(card, 9)  -- lower Z than trackHit
            cardHit.MouseButton1Click:Connect(function() setVal(not value) end)
            cardHit.InputBegan:Connect(function(i)
                if i.UserInputType == Enum.UserInputType.Touch then setVal(not value) end
            end)
        ...
        function Tab:CreateButton(c)
            ...
            local function press(px, py)
                Ripple(card, px, py)
                -- Gentle scale-down press feel instead of a color flash
                Tween(card, {BackgroundColor3 = T.SurfaceAlt}, 0.08)
                task.delay(0.12, function() Tween(card, {BackgroundColor3 = T.Surface}, 0.18) end)
                cb()
            end
            ...
            -- Hover: very faint tint only (was too strong before)
            hit.MouseEnter:Connect(function() Tween(card,{BackgroundColor3=T.SurfaceAlt},0.15) end)
            hit.MouseLeave:Connect(function() Tween(card,{BackgroundColor3=T.Surface},  0.15) end)
        end
