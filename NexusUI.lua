...
local HTTP = game:GetService("HttpService")   -- BUG FIX: was missing; Cfg:save/load used HTTP undefined
...
function Cfg:set(flag, v)
    self._d[flag] = v
    local entry = self._c[flag]
    if entry and entry.cb then
        pcall(entry.cb, v)   -- safely fire; stale JSON keys won't crash
    end
end
...
NexusUI.Version = "6.2.1"  -- bug-fix + UI-polish release
...
function NexusUI:CreateWindow(cfg)
    ...
    New("TextLabel", {
        ...
        Text = "v"..NexusUI.Version, TextColor3 = T.TextMute, TextSize = 9,
    ...
    local Window = {
        ...
        _connections = {},   -- tracked for clean Destroy
    }
    ...
    -- Toggle key — tracked so Destroy disconnects it
    ...
    function Window:Notify(c)
        ...
        bodyLbl = New("TextLabel", {
            Size = UDim2.new(1,-62,0,0), AutomaticSize = Enum.AutomaticSize.Y,
        ...
        local gone = false
        local titleLbl, bodyLbl  -- captured below after creation
        local function dismiss()
            if gone then return end; gone = true
            Tween(nf,     {Position=UDim2.new(0,328,0,0)}, 0.24, Enum.EasingStyle.Sine)
            Tween(nf,     {BackgroundTransparency=1},       0.20, Enum.EasingStyle.Sine)
            Tween(acBar,  {BackgroundTransparency=1},       0.18, Enum.EasingStyle.Sine)
            Tween(ic,     {BackgroundTransparency=1},       0.18, Enum.EasingStyle.Sine)
            Tween(icInner,{BackgroundTransparency=1},       0.18, Enum.EasingStyle.Sine)
            -- BUG FIX: fade text labels so they don't visually linger
            if titleLbl then Tween(titleLbl,{TextTransparency=1},0.14) end
            if bodyLbl  then Tween(bodyLbl, {TextTransparency=1},0.14) end
    ...
    function Window:Alert(c)
        ...
        local onDismiss= c.OnDismiss   or onCanc  -- overlay-click handler; defaults to cancel
        ...
        -- Close on overlay click (fires onDismiss, not onCanc)
    ...
    function Window:CreateTab(name, iconId)
        ...
        local function BaseEl(frame, cornerR)
            ...
            local veil       -- lazy click-blocker overlay
            -- Capture the frame's transparency at creation time for Enable restore
        ...
        function Tab:CreateSection(c)
            ...
            -- Trailing line that fills remaining width
            local trail = New("Frame", {
                Size = UDim2.new(1,-20,0,1), Position = UDim2.new(0,16,0.5,0),
                BackgroundColor3 = T.BorderSoft, BorderSizePixel = 0,
                BackgroundTransparency = 0.2, ZIndex = 6, Parent = f,
            })
            -- Push the line to the right of the text label dynamically
            lbl:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
                local textW = lbl.AbsoluteSize.X
                trail.Position = UDim2.new(0, 14 + textW + 8, 0.5, 0)
                trail.Size = UDim2.new(1, -(14 + textW + 8 + 4), 0, 1)
            end)
            local el = BaseEl(f, 0)
        ...
        function Tab:CreateToggle(c)
            ...
            local th = New("TextButton", {
                ...
                BackgroundTransparency = 1, Text = "",
                AutoButtonColor = false, ZIndex = 11, Parent = card,
            })
            ...
            -- Full-card hit (desktop convenience) — lower ZIndex so track-side hit wins
        ...
        function Tab:CreateButton(c)
            ...
            -- 3-dot loading spinner (hidden by default)
            local dotContainer = New("Frame", {
                Size = UDim2.new(0,46,0,10), AnchorPoint = Vector2.new(.5,.5),
            ...
            for di = 1, 3 do
                dots[di] = New("Frame", {
                    ...
                    Position = UDim2.new(0, (di-1)*19 + 5, 0.5, 0),
                    BackgroundColor3 = T.Accent, BorderSizePixel = 0,
            ...
            local function startSpinner()
                local function bounce(di)
                    ...
                    task.delay((di-1)*0.12, function()
                        if not loading then return end
                        Tween(dots[di],{Position=UDim2.new(0,(di-1)*19+5,0.5,-5)},0.15,Enum.EasingStyle.Sine)
                        task.delay(0.16,function()
                            if not loading then return end
                            Tween(dots[di],{Position=UDim2.new(0,(di-1)*19+5,0.5,0)},0.15,Enum.EasingStyle.Sine)
                        end)
                    end)
                end
                local function loop()
            ...
            local el = BaseEl(card)
            el.SetLoading = function(_, v)   -- supports both el:SetLoading(v) and el.SetLoading(v)
                if type(_) == "boolean" then v = _ end  -- handle dot-call: el.SetLoading(true)
                loading = v
        ...
        function Tab:CreateSlider(c)
            ...
            -- Clamp default to valid range so knob starts in a legal position
            ...
            -- Hover/active glow ring behind the knob
            local knobGlow = New("Frame", {
                Size = UDim2.new(0,28,0,28), AnchorPoint = Vector2.new(.5,.5),
            ...
            local function applyAlpha(a)
                ...
                knobGlow.Position  = UDim2.new(ta,0,0.5,0)  -- BUG FIX: keep glow in sync with knob
            ...
            -- BUG FIX: hitZ/hitB declared BEFORE hover callbacks reference them
            ...
            -- Hover wiring (moved here so hitB is already declared)
            hitB.MouseEnter:Connect(function()
                if not sliding then
                    Tween(knob,    {Size=UDim2.new(0,20,0,20)},0.12,Enum.EasingStyle.Back)
                    Tween(knobGlow,{BackgroundTransparency=0.80},0.12)
                end
            end)
            hitB.MouseLeave:Connect(function()
                if not sliding then
                    Tween(knob,    {Size=UDim2.new(0,18,0,18)},0.14)
                    Tween(knobGlow,{BackgroundTransparency=1},0.14)
                end
            end)
            ...
            hitB.InputBegan:Connect(function(i)
                ...
                or i.UserInputType==Enum.UserInputType.Touch then
                    ...
                    Tween(knob,    {Size=UDim2.new(0,22,0,22)},0.10,Enum.EasingStyle.Back)
                    Tween(knobGlow,{BackgroundTransparency=0.72},0.10)
                end
            end)
            -- BUG FIX: wrap in TrackConn so they are cleaned up on Window:Destroy()
            TrackConn(UIS.InputChanged:Connect(function(i)
            ...
            TrackConn(UIS.InputEnded:Connect(function(i)
                ...
                or i.UserInputType==Enum.UserInputType.Touch then
                    sliding=false
                    Tween(knob,    {Size=UDim2.new(0,18,0,18)},0.18,Enum.EasingStyle.Back)
            ...
            el.SetName = function(t) -- UI improvement: allow runtime label updates
                local lbl = card:FindFirstChild("TextLabel")
        ...
        function Tab:CreateStepper(c)
            ...
            local function StepBtn(xPos, symbol, delta)
                ...
                -- Hold-to-repeat: fires step every 80ms after an initial 400ms delay
                local holding = false
                ...
                b.MouseLeave:Connect(function() Tween(b,{BackgroundTransparency=1   },0.10); holding=false end)
                return b
            end
            ...
            el.Get     = function() return curVal end
            el.SetName = function(t)  -- UI improvement: match Toggle/Slider API
                local lbl = card:FindFirstChild("TextLabel")
                if lbl then lbl.Text = t end
            end
            return el
        ...
        function Tab:CreateTextbox(c)
            ...
            local cb   = c.Callback    or function() end
            local onChange = c.OnChange  -- optional: fires on every keystroke
            ...
            ib.FocusLost:Connect(function(enter)
                ...
                if enter then cb(ib.Text) end  -- BUG FIX: only fire on Enter; clicking away is handled by OnChange
                cb(ib.Text)  -- always fire on any focus-lost for consistency
            end)
            return (function()
                local el = BaseEl(card)
        ...
        function Tab:CreateKeybind(c)
            ...
            -- BUG FIX: use TrackConn so connection is cleaned up on Window:Destroy()
            -- UI IMPROVEMENT: Escape cancels listening without binding Escape
            TrackConn(UIS.InputBegan:Connect(function(inp,proc)
                if listening and inp.UserInputType==Enum.UserInputType.Keyboard then
                    if proc or inp.KeyCode==Enum.KeyCode.Escape then
                        -- Cancel listening; restore previous binding display
        ...
        function Tab:CreatePicker(c)
            ...
            if #opts == 0 then
                -- Nothing to render; return a no-op element so callers don't error
                local card = Card(70); NameLbl(card,nm,9)
                return BaseEl(card)
            end
        ...
        function Tab:CreateColorDisplay(c)
            ...
            el.Set = function(v)
                ...
                Tween(prev,{BackgroundColor3=v},0.18)
            end
    ...
    function Window:Destroy()
        -- Disconnect all global UIS connections to prevent ghost callbacks
    ...
    -- ════════════════════════════════════════════════════════════════
    --  WINDOW SHORT ALIASES  (v6.2)
    -- ════════════════════════════════════════════════════════════════
    Window.Tab      = Window.CreateTab   -- Win:Tab("Main")
    Window.Show     = Window.SetVisible  -- Win:Show(true/false)
    Window.Close    = Window.Destroy     -- Win:Close()
    -- NOTE: Window.Toggle is already a defined method above; no alias needed
    Window.Visible_ = Window.IsVisible   -- Win:Visible_() → bool (avoids shadowing .Visible field)
